...
export default function EmployeeCompensationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function openCreate() {
    setEditing(null);
    // เติมค่าเริ่มต้นจากประวัติค่าจ้างล่าสุด (items เรียงจาก effective_from เรียงลง) เพื่อให้ปรับต่อได้ง่าย เช่นตอนขึ้นเงินเดือน
    const last = items[0];
    const defaultProfile = last
      ? profiles.find((p) => p.id === last.compensation_profile_id)
      : profiles.find((p) => p.is_default) ?? profiles[0];
    setForm({
      ...empty,
      compensation_profile_id: defaultProfile ? String(defaultProfile.id) : "",
      base_salary: last?.base_salary ?? "",
    });
    setErr(null);
    setShowForm(true);
  }
