...
import { Loader2, Check, X, AlertCircle, FileText, Plus, Banknote, MinusCircle } from "lucide-react";
...
export default function AdvanceApprovalPage() {
  ...
  async function action(req: EmployeeAdvance, verb: "approve" | "reject" | "mark-paid") {
    let note: string | null = null;
    if (verb === "reject") {
      note = prompt("เหตุผลที่ปฏิเสธ");
      if (note === null) return;
    }
    if (verb === "mark-paid" && !confirm(`ยืนยันว่าได้จ่ายเงิน ${fmtMoney(req.amount)} บาท ให้พนักงานแล้ว?`)) return;
    setBusy(req.id);
    setErr(null);
    try {
      await apiFetch(`/advances/${req.id}/${verb}`, {
        method: "POST",
        body: verb === "mark-paid" ? undefined : { note },
      });
      setDetail(null);
      await load();
    } catch (e: unknown) {
    ...
    <>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-1 border-b border-border overflow-x-auto">
          ...
          <button
            onClick={() => { setCreateForm(emptyCreate); setCreateErr(null); setShowCreate(true); }}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setDetail(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {detail.status === "approved" && (
              <div className="flex justify-end gap-2 px-5 py-3 border-t border-border bg-gray-50">
                <button
                  onClick={() => action(detail, "mark-paid")}
                  disabled={busy === detail.id}
                  className="px-4 py-2 text-sm rounded-lg bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50 inline-flex items-center gap-2"
                >
