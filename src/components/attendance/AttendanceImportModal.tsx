...
export default function AttendanceImportModal({ open, onClose, onSuccess }: Props) {
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      await downloadReport(
        ...
        `attendance_import_template_${new Date().toISOString().slice(0, 10)}.xlsx`
      );
    } catch (e) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await apiFetch<ImportResult>("/attendance/import", {
        method: "POST",
        body: fd,
      });
      setResult(res);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h3 className="text-lg font-semibold text-foreground">นำเข้าเวลาทำงานจาก Excel</h3>
          <button
            onClick={handleClose}
            disabled={uploading}
        ...
        <div className="p-5 space-y-5 overflow-y-auto">
          ...
          <div className="rounded-xl border border-border p-4">
            ...
            <p className="text-xs text-muted mb-3">
              ไฟล์ template มี 2 sheet: &quot;Template&quot; สำหรับกรอกข้อมูล และ &quot;รหัสพนักงาน&quot;
              สำหรับดูรหัสพนักงานทั้งหมด
              <br />
              คอลัมน์: <span className="font-mono text-foreground">employee_code</span>,{" "}
              <span className="font-mono text-foreground">date</span> (YYYY-MM-DD),{" "}
              <span className="font-mono text-foreground">check_in</span>,{" "}
              <span className="font-mono text-foreground">check_out</span> (HH:MM),{" "}
              <span className="font-mono text-foreground">note</span>
              <br />
              <span className="text-amber-600">หมายเหตุ:</span> ระบบจะคำนวณ &quot;สาย/ออกก่อน/OT&quot;
              จากกะของพนักงานให้อัตโนมัติ และจะลบแถวตัวอย่างก่อนนำเข้าข้อมูลจริง
            </p>
            <button
